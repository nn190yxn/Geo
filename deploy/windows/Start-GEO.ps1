...
function Assert-DockerReady {
  if (-not (Get-Command docker -ErrorAction SilentlyContinue)) {
    throw '未检测到 Docker Desktop。请安装并启动 Docker Desktop 后重新运行 GEO。'
  }

  docker info *> $null
  if ($LASTEXITCODE -ne 0) {
    throw 'Docker Desktop 当前未运行。请启动 Docker Desktop 并等待引擎就绪后重新运行 GEO。'
  }
}
