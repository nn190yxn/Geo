...
Copy-DirectoryContents $prismaClient (Join-Path $app 'apps\api\node_modules\@prisma\client')
Copy-DirectoryContents $generatedPrismaClient (Join-Path $app 'apps\api\node_modules\.prisma\client')
# npm ci uses --ignore-scripts for a deterministic payload, so copy the Windows schema engine explicitly.
Copy-DirectoryContents $prismaEngines (Join-Path $app 'apps\api\node_modules\@prisma\engines')
Copy-DirectoryContents $prismaCli (Join-Path $app 'apps\api\node_modules\prisma')
