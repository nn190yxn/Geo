...
发布配置位于 `当前工作区/.github/workflows/windows-release.yml`，安装程序与控制脚本位于 `当前工作区/deploy/windows/`。桌面启动状态先经过 `services-ready`，Electron 确认 Web 页面 `#root` 已渲染后才写入 `running`，Windows E2E 会同时验证 `uiReady` 和 API readiness。安装包构建前还会使用 Windows Electron 加载最终 Web payload，任何页面加载失败、renderer 崩溃、console error 或根节点未挂载都会阻止发布。前端生产依赖使用精确版本，Ant Design 相关依赖保持在单一 vendor chunk。renderer 的加载失败、崩溃及 console 输出位于 `%LOCALAPPDATA%\AI-Brand-Visibility-Assistant\data\logs\desktop.log`。
