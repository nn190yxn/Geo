...
export class SprintMetricsService {
  ...
  calculateMetricSummary(sprint: VisibilitySprint, runs: MonitoringRunDetail[]): VisibilitySprintMetricSummary {
    return calculateSprintMetricSummary(sprint, runs);
  }
}

export function calculateSprintMetricSummary(sprint: VisibilitySprint, runs: MonitoringRunDetail[]): VisibilitySprintMetricSummary {
  const relatedRunIds = new Set(sprint.relatedMonitoringRunIds);
  const sprintRuns = runs.filter((run) => relatedRunIds.has(run.id) && hasRealMonitoringResponse(run) && run.analysis);
